...
// Scrape Search using Puppeteer (handles Cloudflare)
async function scrapeSearch(keyword) {
    ...
    try {
        ...
        // Try using Puppeteer for Cloudflare bypass
        try {
            console.log('[V1] Attempting search with Puppeteer:', keyword);
            ...
            await page.waitForSelector('.chivsrc', { timeout: 10000 }).catch(() => null);

            // Get page content
            html = await page.content();
            await browser.close();
            browser = null;
            console.log('[V1] Successfully loaded page with Puppeteer');
        } catch (puppeteerError) {
            // Fallback to axios if puppeteer fails
            console.log('[V1] Puppeteer failed, trying axios fallback:', puppeteerError.message);
            ...
            try {
                ...
                console.error('[V1] Both Puppeteer and axios failed:', axiosError.message);
        ...
        if (browser) {
            try {
                ...
                console.error('[V1] Error closing browser:', e.message);
            }
        }
    }
}
